...
A minimalist, pure C implementation of LLM inference, designed to demystify modern serving systems like vLLM. This project implements the core components of Llama inference (Attention, RoPE, RMSNorm, MatMul) from scratch.

**Philosophy:** "No C++, No STL, Just Raw C"

## Features

- **Pure CPU Implementation**: Works on Mac/Linux with standard C compilers.
- **Educational Focus**:
  - ASCII Art Visualization of internal model states (Attention scores, KV Cache).
  - Simple, readable implementations of complex kernels.
  - Direct memory management (malloc) to understand data flow.
- **Paged Attention**: Implements KV Cache paging to support continuous batching.
- **HuggingFace Compatible**: Supports loading any Llama-architecture model from HuggingFace (via export script).
...
│   └── kernels/        # CPU Kernel implementations (Attention, Layers)
